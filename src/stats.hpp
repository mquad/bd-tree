...
template<typename K, typename S>
std::vector<K> rank_all_items(const StatMap<K, S> &stats){
...
template<typename K, typename S>
std::vector<K> rank_all_items(const StatMap<K, S> &stats, const std::map<K, double> &parent_scores, const double h_smooth){
